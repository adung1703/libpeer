...
//const char CAMERA_PIPELINE[] = "rtspsrc location=rtsp://172.19.201.195:8554/stream1 latency=0 ! decodebin ! videoconvert ! video/x-raw,format=I420 ! x264enc tune=zerolatency bitrate=250 speed-preset=ultrafast ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream ! appsink name=camera-sink";
...
static void request_periodic_keyframe() {
    while (!g_interrupted) {
        ...
        sleep(2);  // Adjust as needed for your latency requirements
    }
}
